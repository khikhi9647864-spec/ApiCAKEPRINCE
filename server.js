...
let totalExecute = 0;
let cakePrinceServers = new Map(); 

// Tự động nhận diện tên Sea để hiển thị trên dữ liệu JSON cho đẹp
function getSeaName(placeId) {
    const id = String(placeId);
    if (id === "2753915549") return "Cake Prince Sea 1";
    if (id === "4442272183") return "Cake Prince Sea 2";
    if (id === "7449423635") return "Cake Prince Sea 3";
    return `Cake Prince (Place: ${id})`;
}

// 1. Cổng tiếp nhận dữ liệu từ tất cả các Sea gửi lên
app.post('/update-cake-prince', (req, res) => {
...
app.get('/api', (req, res) => {
    const cakePrinceDataArray = Array.from(cakePrinceServers.values());
    res.json(cakePrinceDataArray);
});

// Cơ chế tự động xóa server khỏi danh sách sau 15 phút (Quét dọn mỗi 1 phút)
setInterval(() => {
    const now = Date.now();
    for (let [jobid, data] of cakePrinceServers.entries()) {
        if (now - data.updatedAt > 15 * 60 * 1000) { 
            console.log(`🧹 [Web] Hết thời gian, xóa Server cũ: ${jobid}`);
            cakePrinceServers.delete(jobid);
        }
    }
}, 60000); 
...
app.get('/', (req, res) => {
    ...
    const finalData = {
        ...
        "by": "tranduykhanh",
        "sea_filter": "All Seas",
        "total_cake_prince_servers": cakePrinceDataArray.length,
        "cake_prince_data": cakePrinceDataArray
    };
